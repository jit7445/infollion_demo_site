...
import { Building2, Lightbulb, Coins, ChevronDown, Users, Presentation, HandCoins } from "lucide-react";
...
function AccordionItem({ title, children, isOpen, onClick }: AccordionItemProps) {
  return (
    <div
      className={`border transition-all duration-300 rounded-xl mb-2 overflow-hidden bg-white/5 backdrop-blur-sm ${isOpen ? "border-brand-primary/50 shadow-[0_0_20px_rgba(255,122,48,0.1)] ring-1 ring-brand-primary/20" : "border-white/5 shadow-sm"}`}
    >
      <button
        onClick={onClick}
      ...
      >
        <span
          className={`text-sm tracking-widest font-bold uppercase transition-colors duration-300 ${isOpen ? "text-brand-primary" : "text-current opacity-80"}`}
        >
          {title}
        </span>
        <ChevronDown
          className={`w-5 h-5 transition-transform duration-300 ${isOpen ? "rotate-180 text-brand-primary" : "text-current opacity-40"}`}
        />
      </button>
    ...
    <div ref={containerRef} className="pt-24 pb-32 relative min-h-screen overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-12 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <h1 className="text-2xl lg:text-3xl font-extrabold tracking-[0.4em] text-brand-text uppercase mb-8">
          ...
          {/* Icon Category Jump Links (3D Emerge Animation) */}
          <div className="grid grid-cols-3 gap-8 max-w-4xl mx-auto" style={{ perspective: "1500px" }}>
            {useCaseCategories.map((cat, i) => {
              ...
              return (
                ...
                >
                  <MovingBorder isHovered={isHovered} duration={3000} borderRadius="1.5rem">
                    <div className="w-24 h-24 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center text-brand-primary shadow-2xl transition-all duration-700 group-hover:bg-brand-primary group-hover:text-white group-hover:-translate-y-4 group-hover:rotate-x-12 group-hover:shadow-[0_20px_40px_rgba(236,147,36,0.4)]">
        ...
        <div className="mt-24 space-y-32">
          {Object.entries(contentMap).map(([id, category], catIdx) => (
            <ScrollZoomWrapper key={id}>
              <div id={id} className="w-full flex flex-col">
                <div className="text-center mb-12">
                  <ScrollTextReveal 
                    text={category.title} 
                    as="h2"
                    className="text-xl font-bold tracking-[0.4em] text-brand-text uppercase mb-4 justify-center"
                  />
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                  {/* Accordion Side */}
                  <div
                    className={`flex flex-col gap-3 ${category.items.length >= 5 ? "h-auto" : "h-auto"} order-2 ${catIdx % 2 !== 0 ? "lg:order-2" : "lg:order-1"}`}
                  >
                  ...
                  {/* Image/Icon Side */}
                  <div
                    className={`relative w-full aspect-[16/11] min-h-[480px] rounded-[40px] bg-brand-text/5 border border-brand-text/10 flex items-center justify-center p-4 order-1 ${catIdx % 2 !== 0 ? "lg:order-1" : "lg:order-2"}`}
                  >
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${category.imageColor} to-transparent opacity-50 transition-opacity rounded-[40px] group-hover:opacity-100`}
                    />
                    <div className="relative z-10 w-full h-full flex items-center justify-center scale-100 transition-transform duration-700 group-hover:scale-110">
                      {category.imageIcon}
                    </div>
